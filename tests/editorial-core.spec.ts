...
test('proposal review shows the source transcript chunk for every shot', async ({
  page,
}) => {
  await page.goto('/');
  ...
  await expect(
    page.getByText('Transcript chunk', { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText(
  ...
  await expect(
    page.getByText('Maybe the premium feature we actually need is an ending.', {
      exact: true,
    }),
  ).toBeVisible();
});

test('proposal review stacks transcript chunks without narrow-screen overflow', async ({
  page,
}) => {
...
test('editorial core keeps the transcript raw until a staged proposal is accepted', async ({
  page,
}) => {
  await page.goto('/');

  await expect(
    page.getByRole('heading', { name: 'Rant Studio' }),
  ).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Propose shots' }).click();
  await expect(
    page.getByRole('heading', { name: 'Propose chronological shots' }),
  ).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Ask attached agent' }).click();
  await expect(
    page.getByText('Codex is reading 84 timestamped words'),
  ).toBeVisible();
  await expect(
    page.getByText('Staged proposal · project unchanged'),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole('button', { name: 'Select Shot 01' }),
  ).toBeVisible();
...
test('editorial core can reject and regenerate without partially applying shots', async ({
  page,
}) => {
  await page.goto('/');
  await page.getByRole('button', { name: 'Propose shots' }).click();
