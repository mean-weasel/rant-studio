...
test('collaboration output oracle preserves human selection and completes preflight', async ({
  page,
}) => {
  ...
  await page
    .getByRole('button', { name: 'Select uploaded candidate for Shot 02' })
    .click();
  await expect(
    page
      .getByTestId('shot-02')
      .getByText('Selected by you · agent cannot replace'),
  ).toBeVisible();

  await page.getByRole('button', { name: 'Select Shot 03' }).click();
  await page
    .getByLabel('Instruction for attached agent')
    .fill('Find a less literal image about the hidden cost.');
  ...
  await expect(
    page.getByText('Candidate attached · awaiting your selection'),
  ).toBeVisible();
  ...
  await expect(
    page.getByText('Find a less literal image about the hidden cost.'),
  ).toBeVisible();
  await expect(
    page.getByText('Added 1 candidate · selection unchanged'),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole('heading', { name: 'Assembled preview' }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole('button', { name: 'Cover landscape visual', pressed: true }),
  ).toBeVisible();
  await page.getByRole('button', { name: 'Close preview' }).click();

  await page.getByRole('button', { name: 'Export' }).click();
  await expect(
    page.getByRole('heading', { name: 'Export preflight' }),
  ).toBeVisible();
  await expect(page.getByText('3 shots need visuals')).toBeVisible();
  await page.getByRole('button', { name: 'Export anyway' }).click();
  await expect(
    page.getByRole('heading', { name: 'Confirm placeholder export' }),
  ).toBeVisible();
...
test('collaboration output supports simulated upload, provenance, and deterministic reset', async ({
  ...
  await expect(
    page.getByRole('heading', { name: 'Attach a candidate' }),
  ).toBeVisible();
